...
extension SemanticInfoBuilder {
    func buildRecursiveParentName(_ info: Syntax) -> String {
        ...
        case .classDecl(let classDecl):
            var finalId = "\(classDecl.name._syntaxNode.strippedText)"
            buildNameWalkingUp(
                from: classDecl,
                targetId: &finalId
            )
        ...
        case .structDecl(let structDecl):
            var finalId = "\(structDecl.name._syntaxNode.strippedText)"
            buildNameWalkingUp(
                from: structDecl,
                targetId: &finalId
            )
        ...
        case .functionDecl(let funcDecl):
            var finalId = "\(funcDecl.name._syntaxNode.strippedText)"
            buildNameWalkingUp(
                from: funcDecl,
                targetId: &finalId
            )
        ...
        case .enumDecl(let enumDecl):
            var finalId = "\(enumDecl.name._syntaxNode.strippedText)"
            buildNameWalkingUp(
                from: enumDecl,
                targetId: &finalId
            )
    ...
    func buildNameWalkingUp(
        ...
        while let next = current {
            ...
            case .classDecl(let outerD):
                targetId = "\(outerD.name._syntaxNode.strippedText).\(targetId)"
            ...
            case .functionDecl(let outerF):
                targetId = "\(outerF.name._syntaxNode.strippedText).\(targetId)"
                
            case .enumDecl(let outerEnum):
                targetId = "\(outerEnum.name._syntaxNode.strippedText).\(targetId)"
                
            case .structDecl(let outStruct):
                targetId = "\(outStruct.name._syntaxNode.strippedText).\(targetId)"
...
private extension SemanticInfoBuilder {
    ...
    func makeTypeAliasInfo(for node: Syntax, fileName: String? = nil, _ typel: TypeAliasDeclSyntax) -> SemanticInfo {
        let aliasName = "\(typel.name) = \(typel.initializer.value.description)"
        return SemanticInfo(
            node: node,
    ...
    func makeVariableInfo(for node: Syntax, fileName: String? = nil, _ varl: VariableDeclSyntax) -> SemanticInfo {
        ...
        if let firstBinding = varl.bindings.first {
            ...
            name = "\(varl.bindingSpecifier.text) \(pattern)\(typeName)"
        } else {
    ...
    func makeClassInfo(for node: Syntax, fileName: String? = nil, _ classl: ClassDeclSyntax) -> SemanticInfo {
        let newInfo = SemanticInfo(
            ...
            fileName: fileName,
            callStackName: "\(classl.name)".trimmingCharacters(in: .whitespaces)
        )
        return newInfo
    }
    ...
    func makeStructInfo(for node: Syntax, fileName: String? = nil, _ structl: StructDeclSyntax) -> SemanticInfo {
        let newInfo = SemanticInfo(
            ...
            fileName: fileName,
            callStackName: "\(structl.name)".trimmingCharacters(in: .whitespaces)
        )
        return newInfo
    }
    ...
    func makeFunctionDeclInfo(for node: Syntax, fileName: String? = nil, _ funcl: FunctionDeclSyntax) -> SemanticInfo {
        let readableFunctionSignataure = "\(funcl.name)\(funcl.signature._syntaxNode.strippedText)"
        let newInfo = SemanticInfo(
            node: node,
            referenceName: readableFunctionSignataure,
            color: CodeGridColors.functionDecl,
            fileName: fileName,
            callStackName: "\(funcl.name)".trimmingCharacters(in: .whitespaces)
        )
        return newInfo
    }
    ...
    func makeProtocolInfo(for node: Syntax, fileName: String? = nil, _ protol: ProtocolDeclSyntax) -> SemanticInfo {
        let newInfo = SemanticInfo(
            node: node,
            referenceName: "\(protol.name)",
            color: CodeGridColors.protocolDecl,
            fileName: fileName,
            callStackName: "\(protol.name)".trimmingCharacters(in: .whitespaces)
        )
        return newInfo
    }
